...
function canBlockAttacker(attackerPiece, attackerRow, attackerCol, attackerColor, targetRow, targetCol)
{
		...
		var enemyPawn = BLACK | PAWN;
		if (attackerColor == "black")
		{
				enemyPawn = WHITE | PAWN;
		}
		...
		for (var i = 1; i < numSteps; i++)
		{
				...
				my_alert("Checking square (" + (targetRow + (i*rowStep)) + "," + (targetCol + (i*colStep)) + ")");

				if (canSquareBeBlocked(targetRow + (i * rowStep), targetCol + (i * colStep), attackerColor))
				{
					my_alert("canBeBlocked( ) -> some piece can move into (" + (targetRow + (i * rowStep)) + "," + (targetCol + (i*colStep)) + ")");
				...
				{ // And for each column
						...
						{ // If the row and col is part of the board..
								...
								{ // If the king is safe to move
										my_alert("King can move into a safe square!");
		...
		if (getNextAttacker(kingRow, kingCol, curColor, attackerCoords))
		{
				...
				if (DEBUG)
				{
						if (canBeBlocked)
								...
								alert("Cannot be blocked");
						if (isEnPrise)
								alert("Can be captured");
						else
								alert("Cannot be captured");
				}
