...
class Message
{
	...
	public function get_outbox_list( )
	{
		...
		if ($result) {
			foreach ($result as $key => $row) {
				...
				$query = "
					SELECT G.*
						, R.p_username AS recipient
					FROM ".self::GLUE_TABLE." AS G
						LEFT JOIN ".T_PLAYER." AS R
							ON (R.p_id = G.to_id)
					WHERE G.message_id = '{$row['message_id']}'
						AND G.to_id <> {$this->_user_id}
					ORDER BY G.to_id ASC
				";
	...
	public function get_admin_list( )
	{
		...
		$query = "
			SELECT G.*
				, IF('' <> M.subject, M.subject, '<No Subject>') AS subject
				, ((G.send_date < NOW( )) || G.send_date IS NULL) AS sent
				, P.p_T_PLAYERusername AS sender
			FROM ".self::GLUE_TABLE." AS G
				LEFT JOIN ".self::MESSAGE_TABLE." AS M
					USING (message_id)
				LEFT JOIN ".T_PLAYER." AS P
	...
	public function get_message($message_id, $admin = false)
	{
		...
		$query = "
			SELECT G.*
				, P.p_username AS recipient
				, S.p_username AS sender
			FROM ".self::GLUE_TABLE." AS G
				LEFT JOIN ".T_PLAYER." AS P
					ON (P.p_id = G.to_id)
				LEFT JOIN ".T_PLAYER." AS S
					ON (S.p_id = G.from_id)
	...
	static public function player_deleted($player_ids)
	{
		...
		$Mysql->delete(self::GLUE_TABLE, " WHERE from_id IN ({$player_ids}) OR to_id IN ({$player_ids}) ");
	}
	...
	protected function _get_message_data($message_id)
	{
		...
		$message['from'] = $message['recipients'][0]['sender'];
		$message['date'] = (empty($message['send_date']) ? $message['create_date'] : $message['send_date']);
		$message['date'] = date($CFG_LONGDATE, strtotime($message['date']));
